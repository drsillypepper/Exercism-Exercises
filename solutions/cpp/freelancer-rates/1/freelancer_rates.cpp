...
int monthly_rate(double hourly_rate, double discount) {
    ...
    double monthly_rate_nodiscount = 22 * daily_cost;
    return std::ceil(apply_discount(monthly_rate_nodiscount, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double daily_cost = daily_rate(hourly_rate);
    int days_covered = budget / apply_discount(daily_cost, discount);
    return days_covered;
